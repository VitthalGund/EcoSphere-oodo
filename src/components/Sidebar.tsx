  ...
  useEffect(() => {
    ...
    const fetchCounts = async () => {
      try {
        ...
        console.warn('Could not load sidebar counts, using seeded fallbacks.', err);
      }
    };
  ...
  const navSections: NavSection[] = [
    ...
    },
    {
      ...
      subItems: [
        ...
        { name: 'Policy Acknowledgements', path: '/governance/acknowledgements' }
      ]
    },
    {
      name: 'Gamification',
    ...
    <aside className="w-64 bg-base border-r border-border h-screen flex flex-col justify-between select-none fixed left-0 top-0 z-20">
      ...
      <div className="p-6 border-b border-border">
        <NavLink to="/" className="flex items-center space-x-3">
          ...
          <span className="text-xl font-bold tracking-tight text-text-primary">EcoSphere</span>
        </NavLink>
      </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div className="truncate w-32">
              <p className="text-xs font-bold text-text-primary truncate">{profile?.name || 'Loading...'}</p>
