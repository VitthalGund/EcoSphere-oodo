...
import { classifyActivityDescription, ClassificationResult } from '../../lib/ai/classifier';
import { CarbonTransaction, Department, EmissionFactor } from '../../lib/types';
import { toast } from 'react-hot-toast';
import { Plus, Search, Edit2, Trash2, X, Filter, Calendar, Sparkles, AlertTriangle, ArrowRight, BrainCircuit } from 'lucide-react';
...
export const CarbonTransactionsPage: React.FC = () => {
  ...
  const [aiResult, setAiResult] = useState<ClassificationResult & { provider: 'gemini' | 'ollama' } | null>(null);
  ...
  const handleAiAnalyze = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err.message || 'AI Classification failed. Check Ollama server state.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleAiApproveAndLog = async () => {
    ...
    const calculatedCo2e = Number((aiResult.amount * Number(factorObj.factor_value)).toFixed(4));
                  ...
                  <tr key={tx.id} className="hover:bg-surface/30 transition-colors">
                    ...
                    <td className="py-4 px-4 text-center">
                      {tx.created_via === 'ai_classifier' ? (
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black bg-[#6741d9]/15 text-[#6741d9] border border-[#6741d9]/10 shadow-xs uppercase tracking-wider" title="Classified by Gemini AI">
          ...
          <div className="bg-base border border-border rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col">
            <div className="p-5 border-b border-border flex items-center justify-between">
              <h3 className="text-sm font-bold text-text-primary uppercase tracking-widest flex items-center space-x-2">
                <span className="text-[#6741d9]"><BrainCircuit className="h-5 w-5 animate-pulse" /></span>
            ...
            <div className="p-5 space-y-4 overflow-y-auto max-h-[500px]">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-t border-b border-border/40 py-3 gap-3 text-left">
                <div className="space-y-0.5">
                  <span className="text-xs font-bold text-text-primary uppercase tracking-wider">Local Fallback Model</span>
                  <p className="text-[10px] text-text-secondary">Ollama fallback used if Gemini API fails or runs offline.</p>
                </div>
                <select
                  value={aiModel}
              ...
              <div className="flex justify-end pt-1">
                ...
                >
                  ...
                  <span>{aiLoading ? 'AI Classification in progress...' : 'Analyze with AI'}</span>
                </button>
              </div>
              ...
              {aiResult && (
                <div className="mt-4 border border-border rounded-xl bg-surface/30 p-4 space-y-4 animate-fade-in text-left">
                  <div className="flex items-center justify-between border-b border-border pb-2.5">
                    <span className="text-xs font-bold text-text-primary uppercase tracking-wider">AI Classification Review</span>
                    ...
                    <div className="flex items-center space-x-2">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black uppercase border ${
                        ...
                        Match: {Math.round(aiResult.confidence * 100)}% Confidence
                      </span>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setAiResult({ ...aiResult, emission_factor_id: e.target.value })}
                        className="w-full px-2 py-1.5 border border-border rounded-lg bg-base text-xs font-semibold focus:outline-none"
                      >
                        <option value="">-- No Match --</option>
                        {factors.map(f => (
                          <option key={f.id} value={f.id}>{f.name} ({f.factor_value} CO₂e)</option>
                        ))}
                      </select>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 text-xs">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setAiResult({ ...aiResult, source_type: e.target.value })}
                        className="w-full px-2 py-1.5 border border-border rounded-lg bg-base text-xs font-bold focus:outline-none"
                      />
                    </div>
                  ...
                  {aiResult.confidence < 0.6 && (
                    <div className="p-3 bg-warning/5 border border-warning/10 rounded-lg flex items-start space-x-2 text-[10px] text-warning leading-relaxed font-semibold">
                      ...
                      <span>Low confidence match. Please verify the matching factor and parsed amount manually.</span>
                    </div>
                  )}
