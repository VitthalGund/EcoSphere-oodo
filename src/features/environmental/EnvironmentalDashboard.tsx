...
export const EnvironmentalDashboard: React.FC = () => {
  ...
  const scopeTotals = transactions.reduce(
    (acc, tx) => {
      ...
      if (
        efName.toLowerCase().includes("fleet") ||
        efName.toLowerCase().includes("gas")
      )
        scope = "Scope 1";
  ...
  const monthlyDataMap = transactions.reduce(
    (acc, tx) => {
  ...
  const pieChartData = [
    {
      name: "Scope 1 (Direct)",
      value: Math.round(scopeTotals["Scope 1"] || 0),
      color: "#e03131",
    }, // Red
    {
      name: "Scope 2 (Indirect)",
      value: Math.round(scopeTotals["Scope 2"] || 0),
      color: "#1971c2",
    }, // Blue
    {
      name: "Scope 3 (Value Chain)",
      value: Math.round(scopeTotals["Scope 3"] || 0),
      color: "#2f9e44",
    }, // Green
  ].filter((item) => item.value > 0);

  // 4. Prepare chart data: Department Bar Chart
  const deptDataMap = transactions.reduce(
    (acc, tx) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-base border border-border rounded-xl p-6 shadow-xs flex items-center justify-between">
          <div className="space-y-1 text-left">
            <span className="text-[10px] font-black uppercase tracking-wider text-text-secondary/50">
              Total Emissions
            </span>
        ...
        <div className="bg-base border border-border rounded-xl p-6 shadow-xs flex items-center justify-between">
          <div className="space-y-1 text-left">
            ...
            <p className="text-xs text-text-secondary">
              {topScopeVal > 0
                ? `${Math.round((topScopeVal / (totalCo2e || 1)) * 100)}% of total footprint`
                : "No data logged"}
            </p>
          </div>
        ...
        <div className="bg-base border border-border rounded-xl p-6 shadow-xs flex items-center justify-between">
          <div className="space-y-1 text-left">
            ...
            <h3
              className={`text-2xl font-black mt-1 flex items-center ${isReduction ? "text-primary" : "text-danger"}`}
            >
              {isReduction ? "-" : "+"}
          ...
          <div
            className={`h-12 w-12 rounded-xl flex items-center justify-center ${isReduction ? "bg-primary/10 text-primary" : "bg-danger/10 text-danger"}`}
          >
            {isReduction ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <Card
            title="Emissions Timeline Analysis"
            subtitle="Breakdown of Scope 1, 2, and 3 emissions (kgCO₂e) over months."
          >
        ...
        <div>
          <Card
            title="Scope Allocation"
            subtitle="Carbon footprint share by emission protocol class."
          >
              ...
              <div className="h-72 w-full mt-4 flex flex-col justify-between">
                <div className="h-56 relative">
                  ...
                  <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
                    <p className="text-[10px] uppercase font-black tracking-widest text-text-secondary/50">
                      Composite
                    </p>
                    <p className="text-lg font-black text-text-primary">
                      {(totalCo2e / 1000).toFixed(1)}t
                    </p>
                  </div>
                </div>
                {/* Labels list */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <Card
            ...
            subtitle="Total CO₂e kg emissions per department."
          >
          ...
          >
            {transactions.length === 0 ? (
              <div className="text-center py-12 space-y-3">
                <p className="text-xs text-text-secondary">
                  No carbon activity has been logged yet.
                </p>
              </div>
            ) : (
