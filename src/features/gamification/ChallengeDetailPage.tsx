...
import { Calendar, Sparkles, Trophy, ArrowLeft, Send, Check, X, ShieldAlert, Award } from 'lucide-react';
...
export const ChallengeDetailPage: React.FC = () => {
  ...
  const [myParticipation, setMyParticipation] = useState<ChallengeParticipation | null>(null);
  ...
  const handleApprove = async (part: ChallengeParticipation) => {
    ...
    try {
      ...
      toast.success(`Submission approved! +${challenge.xp} XP awarded to ${part.employee_name}.`);
          ...
          toast.success(
            (t) => (
              <div className="flex items-center space-x-3 text-left">
                ...
                <div>
                  <p className="font-bold text-text-primary uppercase tracking-wide text-[10px]">New Badge Unlocked!</p>
                  <p className="font-extrabold text-sm text-[#f08c00]">{badge.name}</p>
  ...
  if (error || !challenge) return <ErrorState message={error || 'Challenge not found'} onRetry={loadData} />;

  const isJoined = !!myParticipation;
  const isPendingReview = myParticipation?.approval_status === 'pending' && myParticipation.progress === 100;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          >
            <div className="space-y-6 text-left">
              <div>
                <h4 className="text-xs font-bold text-text-primary uppercase tracking-wider mb-2">Challenge Goals & Details</h4>
                <p className="text-sm text-text-secondary leading-relaxed whitespace-pre-wrap">{challenge.description}</p>
              </div>

              <div className="grid grid-cols-2 gap-6 p-4 rounded-xl border border-border bg-surface/30">
                <div className="space-y-1">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-text-secondary/50">Reward Points</span>
                  <div className="flex items-center space-x-1.5 text-[#f08c00]">
                    <Sparkles className="h-5 w-5 fill-[#f08c00]/10" />
                    <span className="text-lg font-black tracking-wide">+{challenge.xp} XP</span>
                  </div>
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-text-secondary/50">Deadline Date</span>
                  <div className="flex items-center space-x-1.5 text-text-primary">
                    <Calendar className="h-5 w-5 text-text-secondary/60" />
                    <span className="text-sm font-bold">{challenge.deadline}</span>
                  </div>
                </div>
              </div>
              ...
              {isJoined && (
                <div className={`p-4 rounded-xl border flex items-center justify-between ${
                  ...
                  <div className="space-y-0.5 text-xs text-left">
                    <p className="font-bold uppercase tracking-wider">Your Participation Status</p>
                    <p className="text-[11px] opacity-80">
                      {isApproved && `Approved! +${challenge.xp} XP has been added to your profile.`}
                      {isPendingReview && 'Proof submitted. Awaiting manager review.'}
                      {isRejected && 'Your submission was declined. Please verify and submit new proof.'}
                      {!isApproved && !isPendingReview && !isRejected && 'You are enrolled. Complete the goals and submit proof below.'}
                    </p>
                  </div>
                  <StatusBadge status={myParticipation?.approval_status || 'Enrolled'} />
                </div>
              )}
            </div>
          </Card>
            ...
            <div className="space-y-6">
              {!isJoined ? (
                ...
                !isApproved && !isPendingReview && (
                  <Card title="Submit Challenge Verification" subtitle="Provide notes, URL, or code as proof of completion." accent="gamification">
                    <form onSubmit={handleFormSubmit} className="space-y-4 text-left">
                      ...
                      <div className="flex justify-end">
                        ...
                        >
                          ...
                          <span>{isSubmittingProof ? 'Sending...' : 'Submit Verification'}</span>
                        </button>
                      </div>
                    </form>
                  </Card>
        ...
        <div className="lg:col-span-1">
          {isApprover ? (
            <Card title="Manage Submissions" subtitle="Review and approve employee submissions.">
                  ...
                  {participations.map((part) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-text-primary text-xs">{part.employee_name}</span>
                        <StatusBadge status={part.approval_status} className="text-[9px]" />
                      </div>
                      ...
                      {part.proof_url ? (
                        ...
                        <p className="text-[10px] text-text-secondary/60 italic">Joined challenge (no proof submitted yet).</p>
                      )}
            ...
            <Card title="Challenge Progress" subtitle="Key metrics for this challenge.">
              <div className="space-y-4 text-xs text-left">
                <div className="flex items-center justify-between border-b border-border/40 pb-2">
                  <span className="text-text-secondary">Evidence Required</span>
                  <span className="font-bold text-text-primary">{challenge.evidence_required ? 'Yes (Photo/URL)' : 'No'}</span>
                </div>
                <div className="flex items-center justify-between border-b border-border/40 pb-2">
                  <span className="text-text-secondary">Difficulty Rank</span>
                  <span className="font-bold text-text-primary">{challenge.difficulty}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-text-secondary">Participants Enrolled</span>
                  <span className="font-bold text-[#6741d9] bg-[#6741d9]/10 px-2 py-0.5 rounded-full">{participations.length}</span>
                </div>
              </div>
            </Card>
          )}
