...
export const LeaderboardPage: React.FC = () => {
  ...
  const [users, setUsers] = useState<(UserProfile & { department_name?: string; badge_count?: number })[]>([]);
  const [deptScores, setDeptScores] = useState<(DepartmentScore & { employee_count?: number })[]>([]);
  ...
  const deptChartData = deptScores
    .map(d => ({ name: d.department_name, value: d.total_score }));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="text-xs text-text-secondary mt-1">Benchmarking employee XP milestones and department ESG composite scores.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          {activeTab === 'individual' ? (
            /* INDIVIDUAL LIST */
            <Card title="Individual Standings" subtitle="Ranked by total earned XP points.">
              {users.length === 0 ? (
                <div className="text-center py-12 text-xs text-text-secondary/50">No rankings available.</div>
              ) : (
                        ...
                        return (
                          <tr key={u.id} className="hover:bg-surface/30 transition-colors">
                            <td className="py-4 px-4 text-center font-black">
                              {rankMedal ? (
                                <span className="text-lg" title={`Rank ${idx + 1}`}>{rankMedal}</span>
                              ) : (
                                <span className="text-text-secondary/50">#{idx + 1}</span>
                              )}
                            </td>
                            <td className="py-4 px-4">
                              <div className="flex items-center space-x-2.5">
                                <div className="h-7 w-7 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary text-[10px] shadow-inner uppercase">
                                  {u.avatar_url ? (
                                    <img src={u.avatar_url} alt={u.name} className="h-full w-full object-cover rounded-full" />
                                  ) : (
                                    u.name.charAt(0)
                                  )}
                                </div>
                                <span className="font-extrabold text-text-primary">{u.name}</span>
                              </div>
                            </td>
                            <td className="py-4 px-4 text-text-secondary font-medium">{u.department_name}</td>
                            ...
                            <td className="py-4 px-4 text-center font-bold text-text-secondary">Lvl {u.level}</td>
            ...
            /* DEPARTMENT LIST */
            <Card title="Department Benchmarks" subtitle="Ranked by composite ESG scores.">
              {deptScores.length === 0 ? (
                <div className="text-center py-12 text-xs text-text-secondary/50">No department scores generated yet.</div>
              ) : (
                <div className="overflow-x-auto border border-border rounded-xl">
                  <table className="w-full text-xs text-left border-collapse">
                    <thead>
                    ...
                    <tbody className="divide-y divide-border">
                      {deptScores.map((ds, idx) => {
                        ...
                        return (
                          <tr key={ds.id} className="hover:bg-surface/30 transition-colors">
                            <td className="py-4 px-4 text-center font-black">
                              {rankMedal ? (
                            ...
                            <td className="py-4 px-4 font-bold text-text-primary">{ds.department_name}</td>
                            <td className="py-4 px-4 text-center font-bold text-text-secondary">{ds.employee_count}</td>
                            <td className="py-4 px-4 text-center font-black text-primary">{ds.environmental_score}</td>
                            <td className="py-4 px-4 text-center font-black text-[#1971c2]">{ds.social_score}</td>
                            <td className="py-4 px-4 text-center font-black text-[#6741d9]">{ds.governance_score}</td>
        ...
        <div className="lg:col-span-1">
          {activeTab === 'individual' ? (
            <Card title="Top XP Leaders" subtitle="XP standings for top 5 contributors.">
              {topUsersChartData.length === 0 ? (
                <div className="flex h-56 items-center justify-center text-text-secondary/50 text-xs">No chart data available.</div>
              ) : (
                <div className="h-56 w-full mt-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={topUsersChartData} layout="vertical" margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#dee2e6" horizontal={false} />
                      <XAxis type="number" stroke="#495057" fontSize={9} tickLine={false} />
                      <YAxis dataKey="name" type="category" stroke="#495057" fontSize={9} tickLine={false} width={60} />
                      <Tooltip formatter={(v) => `${v.toLocaleString()} XP`} />
                      <Bar dataKey="value" fill="#f08c00" radius={[0, 4, 4, 0]} barSize={12}>
                        {topUsersChartData.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={index === 0 ? '#6741d9' : '#f08c00'} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
            ...
            <Card title="ESG Leaderboard" subtitle="ESG composite scores compared.">
                ...
                <div className="h-56 w-full mt-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={deptChartData} margin={{ top: 10, right: 10, left: -20, bottom: 5 }}>
                      ...
                      <XAxis dataKey="name" stroke="#495057" fontSize={9} tickLine={false} />
                      <YAxis stroke="#495057" fontSize={9} tickLine={false} />
                      <Tooltip formatter={(v) => `${v} ESG`} />
                      <Bar dataKey="value" fill="#2f9e44" radius={[4, 4, 0, 0]} barSize={20}>
                        {deptChartData.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={index === 0 ? '#2f9e44' : '#1971c2'} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
