...
export const classifyActivityDescription = async (
  ...
  const factorsContext = activeFactors
    .map(f => `ID: ${f.id} | Name: ${f.name} | Category: ${f.category} | Unit: ${f.unit}`)
    .join('\n');
...
Return ONLY a valid JSON object in this format (no markdown blocks, no leading/trailing text):
{
  ...
  if (geminiKey && geminiKey.trim() !== '') {
    try {
      ...
      console.warn('Gemini classification failed. Attempting local Ollama fallback...', geminiErr);
    }
  }

  // Fallback to Ollama
  try {
    console.log(`Attempting classification via local Ollama (${preferredLocalModel})...`);
    ...
    throw new Error('AI Classification failed. Both Gemini API and local Ollama fallback were unreachable.');
  }
};
