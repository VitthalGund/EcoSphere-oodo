...
export const recalculateDepartmentScore = async (
  ...
  const totalCo2e = (envData || []).reduce((sum, item) => sum + Number(item.co2e), 0);
  const environmentalScore = Math.max(10, Math.min(100, Math.round(100 - (totalCo2e / 200))));
  ...
  let socQuery = supabase.from('challenge_participations').select('approval_status');
  if (departmentId) {
    ...
    if (userIds.length > 0) {
      ...
      socQuery = socQuery.eq('employee_id', '00000000-0000-0000-0000-000000000000'); // Force empty match
    }
  }
  ...
  if (socData && socData.length > 0) {
    const approved = socData.filter(item => item.approval_status === 'approved').length;
  ...
  if (totalPolicies && totalPolicies > 0) {
    let employeesQuery = supabase.from('users').select('id', { count: 'exact' });
    if (departmentId) {
    ...
    if (totalEmployees && totalEmployees > 0) {
      ...
      let ackQuery = supabase.from('policy_acknowledgements').select('id', { count: 'exact' });
      if (departmentId) {
        const { data: deptUsers } = await supabase
          .from('users')
          .select('id')
        ...
        if (userIds.length > 0) {
          ...
          ackQuery = ackQuery.eq('employee_id', '00000000-0000-0000-0000-000000000000');
        }
      }
...
export const recalculateAllScores = async (period: string = '2026-07'): Promise<void> => {
  try {
    // Get all departments
